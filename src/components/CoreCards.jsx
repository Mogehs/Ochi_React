...
const CoreCards = () => {
  ...
  return (
    <div className=" w-full flex justify-center items-center pb-10 overflow-x-hidden">
      <div
        onClick={handleCards}
        onMouseEnter={() => {
          setYes((yes) => !yes);
        ...
        ref={cardRef}
        className="absolute z-10 w-[73vw] h-[40vw]  bg-transparent rounded-xl cursor-pointer "
      >
        <div
          ref={cursorRef}
          className=" w-[6vw] h-[6vw] pt-[0.3vw] rounded-full absolute   bg-[#CDEA68] uppercase text-center flex items-center justify-center text-zinc-900 text-[1vw] overflow-hidden scale-0  msm:max-md:scale-0"
        >
          Next
        </div>
      </div>
      {cards.map((card, idx) => (
        <div
          className={`core w-[73vw] h-[40vw]  bg-slate-100 rounded-xl cursor-pointer transition-all duration-1000 ${
            isVisible === idx ? "relative" : "absolute opacity-0"
          }`}
        >
          <div className="w-full h-2/3 flex justify-between items-start">
            <div className="logo p-[1.5vw]">
              <svg
            ...
            <div className="object-cover object-center w-[27vw] p-[2vw] ">
              <img className="rounded-md" src={card.image} alt={card.name} />
            </div>
          </div>
